...
export function HorarioDocenteTablero({ horarios }: any) {
      ...
      <Grid container spacing={1}>
        ...
        {horas.map((hora) => (
          <React.Fragment key={hora}>
            {/* Columna de hora */}
            <Grid size={2} sx={{ display: "flex", alignItems: "flex-start", justifyContent: "center", minHeight: 60 }}>
            ...
            {dias.map((dia) => {
              ...
              return (
                <Grid size={2} key={`${dia}-${hora}`} sx={{ position: "relative", minHeight: 60, borderTop: "1px solid #eee" }}>
                  {clases.map((h: any) => {
                    ...
                    return (
                      ...
                      >
                        <Card sx={{ height: "100%", bgcolor: "#1976d2", color: "white", borderRadius: 1 }}>
                          <CardContent sx={{ p: 1 }}>
                            <Typography variant="subtitle2" fontWeight="bold">
                              {h.materiaNombre}
                            </Typography>
                            <Typography variant="body2">{h.seccionNombre}</Typography>
                            <Typography variant="caption">{h.gradoInfo}</Typography>
