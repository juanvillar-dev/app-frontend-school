...
export default function EditarAlumnoPage(): React.JSX.Element {
  ...
  useEffect(() => {
    if (!alumnoId) return;
    fetch(`http://localhost:8080/api/alumnos/${alumnoId}`)
      .then((res) => res.json())
      .then((data) => {
        setFormData({
  ...
  const handleChangeGrado = (e: React.ChangeEvent<HTMLInputElement>) => {
    setFormData({ ...formData, gradoId: parseInt(e.target.value, 10) });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/api/alumnos/${alumnoId}`, {
